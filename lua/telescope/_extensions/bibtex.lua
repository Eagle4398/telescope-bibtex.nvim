...
local function get_context_bib_files()
    local found_files = {}
    context_files = {}
...
local function get_global_bib_files()
    for _, file in pairs(user_files) do
        local p = path:new(file)
        if p:is_dir() then
            get_local_bib_files(file, true)
        elseif p:is_file() then
            if not utils.file_present(non_context_files, file) then
                table.insert(non_context_files, { name = file, mtime = 0, entries = {}, is_global = true })
            end
        end
    end
end
...
local function setup_picker(context, context_fallback)
    if not files_initialized then
        if context then
            ...
            if #non_context_files == 0 and context_fallback then
                get_local_bib_files()
            end
            if copyIntoLocal == true then
                get_global_bib_files()
            end
        else
            get_local_bib_files()
            get_global_bib_files()
    ...
    for _, f in ipairs(context_files) do
        table.insert(current_files, f)
    end
    for _, f in ipairs(non_context_files) do
        if not utils.file_present(current_files, f.name) then
            table.insert(current_files, f)
        end
    end

    bibtex_actions.fallback_opts.current_files = current_files
...
local function bibtex_picker(opts)
    ...
    bibtex_actions.fallback_opts = {
        ...
        copyIntoLocal = opts.copyIntoLocal,
        opts = opts
    }
    if bibtex_actions.fallback_opts.copyIntoLocal == nil then
        bibtex_actions.fallback_opts.copyIntoLocal = copyIntoLocal
    end
...
return telescope.register_extension({
    setup = function(ext_config)
        ...
        if ext_config.copyIntoLocal ~= nil then
            copyIntoLocal = ext_config.copyIntoLocal
        end
